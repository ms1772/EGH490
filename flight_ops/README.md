...
| `preflight_check.py` | S4. One PASS/FAIL table; gates props-on. *(not yet built)* |
| `session_bringup.sh` | S7. The single tmux launcher, identical off-site and in the lab. *(not yet built)* |
| `nodes/vrpn_to_rigidbodies.py` | S6. VRPN `PoseStamped` -> `mocap4r2_msgs/RigidBodies`. *(not yet built)* |
| `nodes/volume_guard.py` | S2. Safety-critical watchdog. *(not yet built)* |
| `nodes/fake_mocap.py` | Synthetic mocap with fault injection, for rig R1. *(not yet built)* |
| `lab_config/expected_state.yaml` | Refuse-to-proceed gates + the managed-parameter allow-list. |
| `lab_config/px4_indoor_params.yaml` | S5. Indoor failsafe/envelope values. *(not yet built)* |
